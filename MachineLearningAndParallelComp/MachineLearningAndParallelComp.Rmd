...
ggplot( r$pred$data, aes( x = truth, y = prob.E3.25 ) )
```
...
tune.wrap <- makeTuneWrapper(lrn, rdesc, par.set = ps, control = ctrl)
```

```{r wrap2}
r2 <- resample(learner = lrn, task = task, resampling = rdesc, show.info = FALSE)
```



TO DO: resample to optimise alpha and lambda,
then resample to estimate MCE
