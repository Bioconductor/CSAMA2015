...
combine LaTeX and `R`), [IPython](http://ipython.org/) for python and
other languages,
...
generation of HTML and PDF documents, although other formats are
available.
...
> as this breaking the support for reproducibility. The final,
...
`html` as final format; replace with `pdf_document` to produce a pdf
report.
...
using `markdown::markdownToHTML` (in case of `knitr::knit2html`) or
the more recent `rmarkdown::render`.
