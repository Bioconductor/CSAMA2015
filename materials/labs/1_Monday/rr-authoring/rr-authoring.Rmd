...
complex analyses, assure reproducibility of the pipeline (any updates
at the data or analysis level are propagated at the report level) and
the final report and to comprehensively communicate these results to
...
single command such as `knitr::knit2html` (that is function
`knit2hrml` from the package `knitr`) or `rmarkdown::render`, or using
editors such as RStudio.

Other types of document and frameworks that combine a programming and
authoring languages are `Sweave` files `Rnw` (that combine LaTeX and
`R`), [IPython](http://ipython.org/) for python and other languages,
...
converts markdown to html and many other formats. This implementation
brings many enhancements to `R` Markdown. We will focus the generation
of HTML and PDF documents, although other formats are available.
...
> and this breaking the support for reproducibility. The final,
...
R Markdown version 2 uses an *optional* header to define the title,
author and output formats of the document. Below, we want to use
`html` as final format; replace with `pdf_document`.
...
1. Both options will first use the `knite::knit` function to *weave*
the document and generate the markdown `md` file that includes the
code outputs.

2. The generation of the final output will render the final document
using `markdown::markdownToHTML` or the more recent `rmarkdown::render`.

> **Exercise:** Experiment with `R` markdown and the features described so
> far and generate the `Rmd` example above. To create your starting
> document, create a new `R Markdown` file using the RStudio menu or
...
> **Exercise:** Experiment with the following syntax using the `Rmd`
> created above.
...
tables can be generated with the helper function `knitr::kable` and
are then displayed accordingly.
