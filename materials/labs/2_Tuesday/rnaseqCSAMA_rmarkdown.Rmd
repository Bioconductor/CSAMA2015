...
<!-- to compile this: library("rmarkdown"); render("rnaseqCSAMA_rmarkdown.Rmd", output_format="all") -->
...
expression workflow, using `r 2+2` along with other
...
**TODO**: here use mapIds() from the *AnnotationDbi* package.
...
bartab(res$padj < alpha, tt$table$FDR < alpha, lvtt$adj.P.Val < alpha, c("DESeq2","edgeR","limma"))

plot(rank(res$padj),rank(tt$table$FDR))
plot(rank(res$padj),rank(lvtt$adj.P.Val))
plot(rank(tt$table$FDR),rank(lvtt$adj.P.Val))
...
alpha <- 0.1
bartab(resLFC$padj < alpha, ttLFC$table$FDR < alpha, lvttLFC$adj.P.Val < alpha, c("DESeq2","edgeR","limma"),skipNone=TRUE)
```
