...
library(png)
im = readPNG("toolpic.png")
grid.raster(im)
```

   - number of genes; distance between objects
...
```{r distdef,fig=TRUE,echo=FALSE,fig.height=4.99}
library(png)
im = readPNG("metricDef.png")
grid.raster(im)
```
...
```{r boo1,fig=TRUE,echo=FALSE,fig.height=4.99}
library(png)
im = readPNG("bootstrap1.png")
grid.raster(im)
```
...
```{r boo2,fig=TRUE,echo=FALSE,fig.height=4.99}
library(png)
im = readPNG("bootstrap2.png")
grid.raster(im)
```
...
```{r boo3,fig=TRUE,echo=FALSE,fig.height=4.99}
library(png)
im = readPNG("bootstrap3.png")
grid.raster(im)
```
