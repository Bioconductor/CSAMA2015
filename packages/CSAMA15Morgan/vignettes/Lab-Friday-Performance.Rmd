...
5. Re-use existing, tested code
   ...
   - Efficient domain-specific implementations, e.g., [snpStats][] for
     GWAS linear models; [limma][] for microarray linear models;
...
3. Use the [microbenchmark][] package to compare performance of `f0()`
   and `f3()` with the original data, `vec`.
